...
class PACEProtocol(
     ...
     */
        ...
        if (MappingType.CAM == mappingType) {
        ...
        try {
            ...
            if ("ECDH" == agreementAlg) {
                ...
                val ephemeralParameters: AlgorithmParameterSpec =
                    mapNonceGMWithECDH(piccNonce, mappingSharedSecretPoint, params as ECParameterSpec)
                ...
                val ephemeralParameters: AlgorithmParameterSpec =
                    mapNonceGMWithDH(piccNonce, Util.os2i(mappingSharedSecretBytes), params as DHParameterSpec)
        ...
        fun pseudoRandomFunction(s: ByteArray, t: ByteArray, p: BigInteger, algorithm: String): ByteArray {
            ...
            try {
                ...
                while (n * l < p.bitLength() + 64) {
                    cipher.init(Cipher.ENCRYPT_MODE, SecretKeySpec(key, 0, k / 8, algorithm), zeroIV)
   ...
   */
        ...
        fun icartPointEncode(t: BigInteger, params: ECParameterSpec?): ECPoint {
            ...
            val x2 =
                b.negate().multiply(onePlusAlphaPlusAlphaSq).multiply(a.multiply(alphaPlusAlphaSq).modPow(pMinus2, p))
                    .mod(p)
            ...
            val xy = if (aaSqTimesH2 == BigInteger.ONE) ECPoint(x2, aa.multiply(h2).mod(p)) else ECPoint(
                x3,
                aa.multiply(u).mod(p)
            )
        ...
        fun encodePublicKeyDataObject(oid: String, publicKey: PublicKey, isContextKnown: Boolean = true): ByteArray {
        ...
        private fun fixDocumentNumber(documentNumber: String): String {
            ...
            val minDocumentNumber = documentNumber.replace('<', ' ').trim { it <= ' ' }.replace(' ', '<')
